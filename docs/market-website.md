...
- **Rarity** comes first from the curated item catalog (see below), which
  assigns the authoritative rarity — including materials, which the suffix
  heuristic alone leaves as "unknown".  For ids the catalog has never seen
  (e.g. brand-new event items) it falls back to the best-effort suffix
  heuristic (`_C/_U/_R/_E/_L`, plus the `_LEG` legendary marker), e.g.
  `Achichorius_E_IV`.  See `aoeo_market.catalog.rarity_of`.
- **Item catalog** — every listing id is resolved against a committed catalog
  (`aoeo_market/data/catalog.json`, generated by `scripts/build_catalog.py`
  from the Project Celeste search database at `ProjectCeleste/celeste-search`),
  which adds the item's display **name**, its **kind** (advisor / blueprint /
  consumable / design / item / material), an **icon** id, the gear **type**
  (e.g. `Fire Pot`), and — where the source records them — a **description**,
  **civilization**, advisor **age**, seasonal **event**, and the crafting
  **recipe** (`school`, `level`, and the ingredients with their quantities).
  These fields ride along on the listing and item
...
- **Crafting cost & dismantling** — for a craftable item the item view lists its
  recipe and prices it: each ingredient's per-unit price is the current median
  while it is listed, else its historical median, and `cost` sums
  `unit_price × quantity` over the ingredients that have a price (with
...
- **Best value (crafting)** — the best-value tab ranks the craftable items we
  have observed by `value_ratio` = the item's unit price ÷ what its recipe's
  ingredients cost at their own unit prices, so above 1 it pays to craft and
  below 1 it pays to buy the item.  Both sides are per unit and priced the way
  the item page prices an ingredient: current median while listed now, else the
