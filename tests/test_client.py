...
def test_default_drain_budget_floored_at_40s():
    ...
    assert client_mod.MarketClient()._default_drain_budget() == MIN_DRAIN_BUDGET == 40.0
    # A longer poll interval still wins over the floor.
    assert client_mod.MarketClient(poll_interval=120.0)._default_drain_budget() == 120.0


def test_next_ctr_wraps_within_one_byte():
    mc = client_mod.MarketClient()
    mc._ctr = 0xFF
...
def test_login_detects_f2_rejection(monkeypatch):
    ...
    mc = client_mod.MarketClient(connect_timeout=1.0)
    with pytest.raises(LoginRejected, match="0xF2"):
        mc.login(client_mod.Session(xuid=1, username="x", token="T" * 32))
...
def test_login_accepts_f2_status_one(monkeypatch):
    ...
    monkeypatch.setattr(client_mod.socket, "create_connection", lambda *a, **k: sock)
    mc = client_mod.MarketClient(connect_timeout=1.0)
    reply = mc.login(client_mod.Session(xuid=1, username="x", token="T" * 32))
...
def test_login_without_reply_is_not_a_rejection(monkeypatch):
    ...
    monkeypatch.setattr(client_mod.socket, "create_connection", lambda *a, **k: sock)
    mc = client_mod.MarketClient(connect_timeout=1.0)
    assert mc.login(client_mod.Session(xuid=1, username="x", token="T" * 32)) == b""
